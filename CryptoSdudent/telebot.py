...
def get_crypto(name):
    mainText = "        Weighted average rate of %s:           \n%s        %s\n  Changes for:    \nday:  %s      \nweek:  %s      \nmonth:  %s     \n6-month:  %s"
    c_url = url + name.lower()
    c_page = open_page(c_url)
    c_data = parse_crypto(c_page)
    c_text = mainText%(name, c_data[0]['price'], c_data[0]['up'], c_data[0]['day'], c_data[0]['week'], c_data[0]['month'], c_data[0]['sixmonth'])
    
    return c_text


bot = telegram.Bot(token='731950894:AAHUdl46u5Ii1praxVqOB6qrr9pXTAyiO0E')
updater = Updater(token="731950894:AAHUdl46u5Ii1praxVqOB6qrr9pXTAyiO0E")
...
def echo(bot, update):
    ...
    chouse = tops[1][idx]
    bot.send_photo(chat_id=update.message.chat_id, photo=open(chouse.lower()+'.png','rb'))
